...
\begin{itemize}
    ...
    \item Размер данных: 
    \begin{itemize}
        \item маленькие (1,000--10,000 операций)
        \item средние (100,000--1,000,000 операций) 
        \item большие (10,000,000+ операций)
    \end{itemize}
    \item Соотношение массивов к элементам: 0.1, 1.0, 10.0
\end{itemize}


\paragraph{1) Маленькие данные (1,000 операций)}
Параметры: 100 элементов, 10 массивов
...
\vspace{5mm} 
\textbf{Объяснение:} При маленьких объемах данных накладные расходы на создание и синхронизацию потоков превышают выгоду от использования параллельных потоков.

\paragraph{2) Средние данные (100,000 операций)}
Параметры: 1,000 элементов, 100 массивов
...
\vspace{5mm} 
\textbf{Объяснение:} Для такого объема данных это оптимальная зона --- хорошее ускорение с приемлемой эффективностью.

\paragraph{3) Большие данные (10,000,000 операций)}
Параметры: 10,000 элементов, 1,000 массивов
...
\vspace{5mm} 
\textbf{Объяснение:} Отличное ускорение, но эффективность падает при 8 потоках из-за ситуации, когда несколько потоков одновременно пытаются получить доступ к одному ресурсу.
...
\begin{itemize}
    \item \textbf{Стратегия 0 (по элементам)} --- лучше когда:
    \begin{itemize}
        \item Элементов значительно больше чем массивов ($\text{ratio} < 0.5$)
    \end{itemize}
    
    \item \textbf{Стратегия 1 (по массивам)} --- лучше когда:
    \begin{itemize}
        \item Массивов значительно больше чем элементов ($\text{ratio} > 2.0$)
        \item Равномерное распределение работы
    \end{itemize}
\end{itemize}
